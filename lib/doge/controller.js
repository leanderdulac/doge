...
module.exports = new Class('Controller', function() {
	...
	this.$.call = function(request, response, next) {
		...
		.then(function() {
			this.action = this.request.pathParameters.action;

			return this.dispatchAction(this.action);
		})
		.catch(function(err) {
	...
	this.metaclass.$._convertHookOptions = function(options) {
		...
		if (options.only) {
			options.if = options.if.concat(_.map(options.only, function(action) {
				return function() {
		...
		if (options.except) {
			options.unless = options.unless.concat(_.map(options.except, function(action) {
				return function() {
					return this.action == action;
				};
			}));
