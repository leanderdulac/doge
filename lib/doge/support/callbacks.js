...
var HookCollection = new Class('HookCollection', function() {
	...
	this.$.add = function(type, hook, options) {
		...
		if (existent) {
			existent.options = this.mergeHook(existent, newHook);
		} else {
			this.hooks[type].push(newHook);
		}
	};
	...
	this.$.mergeHook = function(a, b) {
		...
		options.conditions = (a.options.conditions || []).concat(b.options.conditions || []);
